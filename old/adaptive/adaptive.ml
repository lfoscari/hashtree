...
let build_tree bucket_size table_size feature_amount test_set =
    ...
    let avg_insertion_cost = ( List.fold_left (+) 0 insertion_costs |> float_of_int ) /. ( float_of_int testset_rows_amount ) in

    tree, avg_insertion_cost
...
let test_mb m b generator features_amount = 
    ...
    let trees, insertion_costs = List.map ( build_tree m b features_amount ) csvs |> List.split in
    ...
    let avg_insertion_cost = List.fold_left (+.) 0. insertion_costs /. ( float_of_int testset_rows_amount ) in
...
let run_permutations_test source_file destination_file map =
    ...
    [ "m"; "b"; "avg_depth"; "avg_usage"; "std_depth"; "std_usage"; "avg_access"; "avg_insertion_cost" ] :: test_results
